...
ChristmasTree <- read.csv2("Data.csv",header = TRUE)
...
tree <- tree + 
  geom_point(data = Baubles, aes(x = Bauble.X, y = Bauble.Y, 
             ...
             shape = 16) +
  scale_colour_manual(values = c("firebrick2", "#EE6677", "dodgerblue3")) +
...
tree <- tree +
  annotate("text", x = 11, y = 20, label = "Kevin Christian ZANOU wish you\n Merry Christmas!", 
           family = "Luminari", size = 5.5)
tree




grid.arrange(tree,bottom="Made using the software R")
